...
export class MetadataWorkspace {
  ...
  markup(): string {
    ...
    const content = this.result ? this.resultMarkup(this.result) : `<div class="metadata-empty"><img class="brand-mark brand-icon" src="./app-icon.png" alt=""><h2>Read an image's hidden prompt.</h2><p>Choose or drop a NovelAI Image. Analysis stays entirely on this device and never changes your prompt builder.</p></div>`;
