...
export class CustomTagsWorkspaceModule {
  ...
  private cardSignature(tag: CustomTag, state: CustomTagsWorkspaceState): string {
    ...
    return JSON.stringify({ id: tag.id, tag: tag.tag, kind: tag.kind, zone: tag.zone, description, image, status, canReorder });
  }
  ...
  private card(tag: CustomTag, state: CustomTagsWorkspaceState): string {
    ...
    const move = this.moveMarkup(tag, state);
  ...
  refresh(_controller: WorkspaceController, afterPatch?: (scope: ParentNode) => void): void {
    ...
    const library = grid.closest('.custom-tag-library'); const heading = library?.querySelector<HTMLElement>('.subheading h3'); if (heading) heading.innerHTML = `${cards.length} shown <span>·</span> ${s.tags.length} total`;
