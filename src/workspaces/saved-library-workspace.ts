...
import { bindArtistCardPreview, clearArtistCardPreview } from '../artist-card-preview.ts';
...
export class SavedLibraryWorkspaceModule {
  ...
  private card(item: SavedLibraryItem, state: SavedLibraryWorkspaceState): string {
    ...
    const preview = image ? ` tabindex="0" role="img" aria-label="Preview cover for ${e(item.name)}" data-library-preview-image="${e(image)}" data-library-preview-tag="${e(item.name)}"` : '';
    const signature = JSON.stringify([item.updatedAt, item.name, item.description, item.kind, image]);
    const apply = item.kind === 'prompt' ? `<button class="primary saved-library-rounded saved-library-use" type="button" data-use-library="${e(item.id)}">Use in Builder</button>` : '';
    return `<article class="saved-library-card" data-saved-library-card="${e(item.id)}" data-saved-library-signature="${e(signature)}"><div class="saved-library-cover${image ? ' has-image' : ''}"${preview}>${image ? `<img data-preview-cache="content" data-preview-src="${e(image)}" alt="" loading="lazy">` : '<span aria-hidden="true">✦</span>'}</div><div class="saved-library-card-body"><div class="saved-library-card-heading"><div><p class="eyebrow">${label}</p><h3>${e(item.name)}</h3></div><time datetime="${e(item.updatedAt)}">${e(new Date(item.updatedAt).toLocaleDateString())}</time></div><p class="saved-library-description">${e(item.description || 'No description.')}</p>${details}<div class="saved-library-actions">${apply}<button class="secondary saved-library-rounded" type="button" data-edit-library="${e(item.id)}">Edit</button><button class="secondary saved-library-rounded" type="button" data-delete-library="${e(item.id)}">Delete</button></div></div></article>`;
  }
  private createPromptCard(): string {
    return `<button class="saved-library-create-card" id="save-library-prompt" type="button"><span class="saved-library-create-icon" aria-hidden="true">＋</span><span><b>New Prompt</b><small>Create an independent prompt record.</small></span></button>`;
  }
  private emptyState(state: SavedLibraryWorkspaceState): { title: string; copy: string } {
    return state.items.length
      ? { title: 'No saved items match this search.', copy: 'Try a different name or type filter.' }
      : { title: 'Your Saved Library is ready.', copy: 'Use New Prompt to create a Prompt, Artist Mix, or Character record.' };
  }
  private emptyMarkup(state: SavedLibraryWorkspaceState): string {
    const empty = this.emptyState(state);
    return `<div class="saved-library-empty"><span class="brand-mark">✦</span><h3>${empty.title}</h3><p>${empty.copy}</p></div>`;
  }
  private grid(state = this.state()): string {
    const items = this.visible(state); const empty = !items.length ? this.emptyMarkup(state) : '';
    return `${this.createPromptCard()}${items.map(item => this.card(item, state)).join('')}${empty}`;
  }
  markup(): string {
    ...
    return `<section id="saved-library-panel" class="${s.panelClass} saved-library-workspace" role="tabpanel" aria-labelledby="saved-library-tab"><header class="workspace-intro"><div><p class="eyebrow">PERSONAL LIBRARY</p><h2 id="saved-library-title">Saved Library</h2><p>Create independent prompt, Artist Mix, and Character records on this device.</p></div></header><div class="saved-library-tools"><input id="saved-library-search" value="${e(s.search)}" placeholder="Search saved items..." aria-label="Search Saved Library"><div class="saved-library-filter" role="group" aria-label="Filter Saved Library">${filters.map(([value, label]) => `<button class="chip ${s.filter === value ? 'on' : ''}" type="button" data-library-filter="${value}" aria-pressed="${s.filter === value}">${label}</button>`).join('')}</div></div><div class="saved-library-grid" id="saved-library-grid">${this.grid(s)}</div></section>`;
  }
  refresh(_controller: WorkspaceController, afterPatch?: (scope: ParentNode) => void): void {
    const s = this.state(); const grid = document.querySelector<HTMLElement>('#saved-library-grid');
    if (!grid) return;
    const items = this.visible(s); const scrollTop = grid.scrollTop;
    const directChildren = [...grid.children].filter((node): node is HTMLElement => node instanceof HTMLElement);
    let create: HTMLElement | null = directChildren.find(node => node.id === 'save-library-prompt') ?? null;
    if (!create) {
      const template = document.createElement('template'); template.innerHTML = this.createPromptCard();
      create = template.content.firstElementChild as HTMLElement | null;
      if (create) grid.prepend(create);
    } else grid.prepend(create);
    const existing = new Map(directChildren.filter(node => node.hasAttribute('data-saved-library-card')).map(node => [node.dataset.savedLibraryCard ?? '', node]));
    const wanted = new Set(items.map(item => item.id));
    const present = new Set(s.items.map(item => item.id));
    let previewSurfaceChanged = false;
    for (const item of items) {
      const signature = JSON.stringify([item.updatedAt, item.name, item.description, item.kind, s.imageUrl(item)]);
      const prior = existing.get(item.id);
      if (prior?.dataset.savedLibrarySignature === signature) { prior.hidden = false; grid.append(prior); continue; }
      if (prior) previewSurfaceChanged = true;
      const template = document.createElement('template'); template.innerHTML = this.card(item, s);
      const replacement = template.content.firstElementChild as HTMLElement | null;
      if (!replacement) continue;
      replacement.dataset.savedLibrarySignature = signature;
      replacement.hidden = false;
      if (prior) prior.replaceWith(replacement);
      grid.append(replacement);
    }
    for (const [id, node] of existing) {
      if (!present.has(id)) { previewSurfaceChanged = true; node.remove(); }
      else if (!wanted.has(id)) node.hidden = true;
    }
    const empty = [...grid.children].find((node): node is HTMLElement => node instanceof HTMLElement && node.classList.contains('saved-library-empty'));
    if (!items.length) {
      const emptyState = this.emptyState(s);
      if (!empty) { const template = document.createElement('template'); template.innerHTML = this.emptyMarkup(s); const next = template.content.firstElementChild; if (next) grid.append(next); }
      else { const title = empty.querySelector('h3'); const copy = empty.querySelector('p'); if (title) title.textContent = emptyState.title; if (copy) copy.textContent = emptyState.copy; }
    } else empty?.remove();
    grid.scrollTop = scrollTop;
    document.querySelectorAll<HTMLButtonElement>('[data-library-filter]').forEach(b => { const on = b.dataset.libraryFilter === s.filter; b.classList.toggle('on', on); b.setAttribute('aria-pressed', String(on)); });
    if (previewSurfaceChanged) clearArtistCardPreview();
    bindArtistCardPreview(grid);
    afterPatch?.(grid);
  }
  route(event: Event): boolean {
    ...
    if (button.id === 'save-library-prompt') { this.actions.create('prompt'); return true; } if (button.id === 'save-library-mix') { this.actions.create('artist-mix'); return true; } if (button.id === 'save-library-character') { this.actions.create('character'); return true; }
    if (button.dataset.useLibrary) { this.actions.apply(button.dataset.useLibrary); return true; }
