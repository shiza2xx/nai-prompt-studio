...
/** Return the maximum companion count that fits the native Artist Mix workspace. */
...
export function mixOrbitLayout(total: number, anchorCount = 1, bounds: MixOrbitBounds = {}): MixOrbitLayout {
  const anchors = Math.max(1, Math.min(4, Math.floor(Number(anchorCount) || 1)));
  ...
  const anchorBox: OrbitBox = { left: Math.round((width - anchorWidth) / 2), top: Math.round((height - anchorHeight) / 2), width: anchorWidth, height: anchorHeight };
  ...
  const solve = (companionWidth: number, companionHeight: number): MixOrbitPlacement[] => {
    ...
    for (let row = 0; row < rows; row += 1) for (let column = 0; column < columns; column += 1) {
      ...
      if (intersects(box, anchorBox)) continue;
  ...
  let density: 'regular' | 'compact' = 'regular';
